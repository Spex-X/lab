...
ALTER TABLE raffles ADD COLUMN IF NOT EXISTS prize_value numeric;
ALTER TABLE raffles ADD COLUMN IF NOT EXISTS draw_date timestamp with time zone;
ALTER TABLE raffles ADD COLUMN IF NOT EXISTS status text DEFAULT 'active';
ALTER TABLE raffles ADD COLUMN IF NOT EXISTS updated_at timestamp with time zone DEFAULT timezone('utc'::text, now()) NOT NULL;
...
ALTER TABLE orders ADD COLUMN IF NOT EXISTS mercado_pago_external_reference text;
...
ALTER TABLE tickets ADD COLUMN IF NOT EXISTS order_id uuid REFERENCES orders(id) ON DELETE SET NULL;
ALTER TABLE tickets ADD COLUMN IF NOT EXISTS reserved_until timestamp with time zone;
ALTER TABLE tickets ADD COLUMN IF NOT EXISTS purchased_at timestamp with time zone;
ALTER TABLE tickets ADD COLUMN IF NOT EXISTS buyer_id uuid REFERENCES profiles(id);
...
DROP POLICY IF EXISTS "Creators can delete their raffles" ON raffles;
CREATE POLICY "Creators can delete their raffles" ON raffles FOR DELETE USING (auth.uid() = created_by);

DROP POLICY IF EXISTS "Admins can view all raffles" ON raffles;
CREATE POLICY "Admins can view all raffles" ON raffles FOR SELECT
  USING (EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'admin'));

DROP POLICY IF EXISTS "Admins can update all raffles" ON raffles;
CREATE POLICY "Admins can update all raffles" ON raffles FOR UPDATE
  USING (EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'admin'));

DROP POLICY IF EXISTS "Admins can delete all raffles" ON raffles;
CREATE POLICY "Admins can delete all raffles" ON raffles FOR DELETE
  USING (EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'admin'));
...
DROP POLICY IF EXISTS "Creators can insert tickets" ON tickets;
CREATE POLICY "Creators can insert tickets" ON tickets FOR INSERT
  WITH CHECK (
    EXISTS (
...
DROP POLICY IF EXISTS "Creators can delete tickets" ON tickets;
CREATE POLICY "Creators can delete tickets" ON tickets FOR DELETE
  USING (
    EXISTS (
      SELECT 1 FROM raffles
      WHERE raffles.id = tickets.raffle_id
...
DROP POLICY IF EXISTS "Creators can view raffle orders" ON orders;
CREATE POLICY "Creators can view raffle orders" ON orders FOR SELECT
  USING (EXISTS (SELECT 1 FROM raffles WHERE raffles.id = orders.raffle_id AND raffles.created_by = auth.uid()));
...
BEGIN
  ...
  IF v_user_id IS NOT NULL THEN
    ...
    ON CONFLICT (id) DO UPDATE SET
      role = 'admin';
    RAISE NOTICE 'Usuário % promovido a admin!', v_user_email;
  ELSE
    RAISE NOTICE 'Usuário % não encontrado. Crie a conta no sistema primeiro e rode este bloco depois.', v_user_email;
  END IF;
END $$;
